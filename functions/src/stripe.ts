...
export const stripe_config = onRequest({ cors: true, region: "us-east1" }, async (req, res) => {
  try {
    const db = getDb();
    let active = true;
...
export const stripe_checkout = onRequest({ cors: true, region: "us-east1" }, async (req, res) => {
  try {
...
export const stripe_webhook = onRequest({ cors: true, region: "us-east1" }, async (req, res) => {
  try {
    if (req.method !== "POST") {
